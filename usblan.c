 ...
 *
 * By:
 *      Stuart Lynne <sl@belcarra.com>, Bruce Balden <balden@belcarra.com>, Shahrad Payandeh <sp@belcarra.com>
 *
...
typedef enum usbdnet_device_type {
        usbdnet_unknown, usbdnet_basic, usbdnet_cdc, usbdnet_safe, usbdnet_blan, usbdnet_rndis, usbdnet_eem,
} usbdnet_device_type_t;

char * usbdnet_device_names[] = {
        "UNKNOWN", "BASIC", "CDC", "SAFE", "BLAN", "RNDIS", "EEM",
};
...
typedef struct eem_info{
        ...
        int unused_len;
        int crc_check;
        int frame_type; //1:data 2:command
} eem_info_struct;
...
#define COMMUNICATION_EEM_SUBCLASS		0x0c	
#define COMMUNICATION_EEM_PROTOCOL		0x07
...
net_hard_start_xmit(struct sk_buff *skb, struct net_device *net)
{
        ...
        int             length;
//#if defined(CONFIG_USB_USBLAN_TYPE_BLAN)
        __u32           fcs;
        int             pad;
//#endif
//        printk(KERN_INFO "%s:\n",__FUNCTION__);
...
//		printk (KERN_INFO"usblan send without CRC.\n");
	...
	fcs = 0;
	pad = 0;
//	printk (KERN_INFO "EEM packet was sent\n");
        if (priv->bmDataCapabilities & DATA_CRC) {
	
	}
	else {
//	    printk (KERN_INFO"EEM sent without CRC\n");
    	    // allocate a new skb, copy data to it computing FCS, 
            // the extra bytes are for the CRC and 2 byte for header
	    THROW_IF (!(skb2 = alloc_skb(length + 4 + 2, flags)), free_urb_and_skb); // XXX +4 +2?

	    skb_put (skb2, skb->len+4+2);
	    skb2->data[1] = (((skb->len + 4) >> 8) & 0x3f);
	    skb2->data[0] = (skb->len + 4) & 0xff;
	    memcpy (skb2->data+2, skb->data, skb->len);
	    skb2->data[skb->len+2] = 0xde;
	    skb2->data[skb->len+2+1] = 0xad;
	    skb2->data[skb->len+2+2] = 0xbe;	    
	    skb2->data[skb->len+2+3] = 0xef;	    
        ...
        if (echo_tx) {
                int             i;
                printk(KERN_INFO"%s: skb: %p len: %d\n", __FUNCTION__, skb, skb->len);
                for (i = 0; i < skb->len; i++) {
                        if ((i % 32) == 0) {
                                printk("\ntx[%3x]: ", i);
                        }
...
//        printk(KERN_INFO "%s: urb %p len: %d status=%d\n", __FUNCTION__, urb, urb->actual_length,urb->status);
	...
	//checking for ZLP
	//XXX Seems that ZLP dosen't work. It gets always URB_ASYNC_UNLINK instead of URB_ZERO_PACKET
	if (urb->transfer_flags == URB_ZERO_PACKET){
		printk (KERN_INFO"Recieve ZLP\n");
//		return;
		}
	if (urb->actual_length == 0){
		printk(KERN_INFO" Recieve ZLE\n");
//		return;
		}
...
//#if defined(CONFIG_USB_USBLAN_TYPE_BLAN)
	__u32           fcs;
//#endif	
//#if defined(CONFIG_USB_USBLAN_TYPE_EEM)	
	unsigned char  *cp = skb->data;
	static eem_info_struct einfo;
//#endif	
	...
	case usbdnet_eem:
//		printk (KERN_INFO"%s: USBLAN_CDC_EEM\n", __FUNCTION__);

	        //find ZLE
        	if ( !(cp[0] | cp[1]) ){
			einfo.zle_zlp = 1;
			printk(KERN_INFO"Find ZLP\n");
		        return;
			}
		else
			einfo.zle_zlp = 0;
...
//		printk (KERN_INFO"%d %d\n", cp[0], cp[1]);
		//Check type of frame
		if ( !(cp[1] & 0x80) ){	//data type 
//		    printk (KERN_INFO "Data type frame\n");
		    if ( cp[1] & 0x40 ){	//CRC check
		    
		    }	//end of CRC check
		    else {	//NO CRC oxdeadbeef
//			printk (KERN_INFO "Without CRC\n");
//			printk (KERN_INFO "cp[skb->len-3] ...: %X %X %X %X\n", cp[skb->len-4],
//				cp[skb->len-3], cp[skb->len-2], cp[skb->len-1]);
#if 0
			THROW_IF (  (cp[skb->len-4] != 0xde) ||
				    (cp[skb->len-3] != 0xad) ||
				    (cp[skb->len-2] != 0xbe) ||
				    (cp[skb->len-1] != 0xef), crc_error);		    
#endif				    
			//find the length of data
			
		    }	//end of no CRC
		}
		else if ( cp[1] & 0x80 ){	//command type
		    
		    u8 bmEEMCmd = (cp[1] >> 3) & 0x7;
		    u8 bmEEMCmdParam = ((cp[1] & 0x7) << 8) | cp[0];
		    
		    printk (KERN_INFO"bmEEMCmd: %d bmEEMCmdParam: %06x\n", bmEEMCmd, bmEEMCmdParam);
		...
		printk (KERN_INFO"Recieved length: %d\n", skb->len);
                for (i = 0; i < skb->len; i++) {
                        if ((i % 32) == 0) {
                                printk("\nrx[%3x]: ", i);
                        }
        ...
        case usbdnet_blan:
	CATCH(crc_error) {
		printk(KERN_INFO "Catch the error\n");
                dev_kfree_skb_any(skb);
                priv->stats.rx_errors++;
        }
		break;
 ...
 */
STATIC void
defer_skb(struct private *priv, struct sk_buff *skb, struct skb_cb *cb, skb_state_t state)
{
        ...
        if (!(list = skb->list)) {
//                printk(KERN_ERR "%s: list is NULL!\n", __FUNCTION__);
                return;
        }
#else                                              
	if (state == 2){
...
//		printk (KERN_INFO"priv->txq.lock: %p with state: %d\n", &list->lock, state);
	}
...
//		printk (KERN_INFO"priv->rxq.lock: %p with state: %d\n", &list->lock, state);
        }
...
/*
 * See if we can have a EEM style no-data interface:
 *      
 *      1. Must have specified class, subclass and protocol
 *      2. Must have have BULK-IN and BULK-OUT endpoints
 *      3. May have INTERRUPT endpoint
 *
 */
STATIC int find_interface_eem(
                struct private *priv, 
                struct usb_interface *eem_interface,
                int class, 
                int subclass,
                int protocol,
                char *guid
                )
{
        ...
        if (eem_interface->num_altsetting > 1) {
                printk(KERN_INFO"%s: too many intefaces num_altsetting: %d\n", __FUNCTION__, eem_interface->num_altsetting);
                return -1;
        }

        //interface_descriptor = mdlm_interface->altsetting;
        interface_descriptor = USB_ALTSETTING(eem_interface,0);
	...
	printk (KERN_INFO"%s: Configuration is ok\n", __FUNCTION__);
        if (interface_descriptor->bNumEndpoints) {
                int endpoint_number;
                priv->comm_ep_in = priv->data_ep_in = priv->data_ep_out = 0;
                for (endpoint_number = 0; endpoint_number < interface_descriptor->bNumEndpoints; endpoint_number++) {
...
/*
 * See if we can have an CDC-EEM interface:
 *      
 *      1. class        CDC_INTERFACE_CLASS
 *      2. subclass     COMMUNICATION_EEM_SUBCLASS
 *      3. protocol     COMMUNICATION_EEM_PROTOCOL
 *
 */
STATIC int verify_eem_interface( 
                struct usb_device *device, 
                struct private *priv, 
                struct usb_interface *data_interface
                )
{
	...
	printk (KERN_INFO"%s: start\n", __FUNCTION__);
        if (find_interface_eem(priv, data_interface, 
                                CDC_INTERFACE_CLASS, 
                                COMMUNICATION_EEM_SUBCLASS, 
                                COMMUNICATION_EEM_PROTOCOL, 
                                BLAN_GUID)) 
        {
                printk(KERN_INFO"%s: not found\n", __FUNCTION__);
                return -1;
        ...
        priv->comm_interface = priv->data_interface = 0;
        priv->usbdnet_device_type = usbdnet_eem;
        return 0;
}
...
STATIC int find_valid_configuration(struct usb_device *usbdev, struct private *priv)
{
                        ...
                        configuration_number++ ) 
        {
                ...
                case 1: 
                        ...
                        else if ( !verify_eem_interface(usbdev, priv, USB_CONFIG2IFC(configuration,0))) {
                                printk(KERN_INFO"%s[%d] bConfigurationValue: %d bNumInterfaces: %d\n", __FUNCTION__, 
                                       configuration_number, priv->bConfigurationValue, configuration->bNumInterfaces);
                                printk(KERN_INFO"%s: cdc-eem\n", __FUNCTION__);
                                return 0;
                        }
...
usblan_probe(struct usb_device *usbdev, unsigned int ifnum, const struct usb_device_id *id)
{
	...
	//printk(KERN_INFO"%s: priv->net: %p\n", __FUNCTION__, &priv->net);
	...
	//printk(KERN_INFO"%s: device type (%d)\n", __FUNCTION__, priv->usbdnet_device_type);
        switch (priv->usbdnet_device_type ) {
        case usbdnet_cdc:
        case usbdnet_rndis:
